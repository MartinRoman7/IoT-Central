...
function sendTelemetry(data_file) {
    ...
    var temp_sensor_1 = clear_data[2];
    var temp_sensor_2 = clear_data[4].replace(';','');
    let now = new Date();
    var date_time = date.format(now, 'YYYY/MM/DD HH:mm:ss');
    var temperature = targetTemperature + (Math.random() * 15);
    var data = JSON.stringify({ temperature_01: temp_sensor_1, temperature_02: temp_sensor_2});
...
function onCommandEcho(request, response) {
  ...
  var connectCallback = (err) => {
    if (err) {
      ...
      readTextFile("file:///home/pi/Desktop/cadena-frio/data-IoT/temperatura");
